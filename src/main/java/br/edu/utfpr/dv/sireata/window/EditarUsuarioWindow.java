...
public class EditarUsuarioWindow extends EditarWindow {
	...
	public EditarUsuarioWindow(Usuario usuario, ListView parentView){
		...
		if(Session.isAdministrador()){
			this.adicionarCampo(Arrays.asList( new HorizontalLayout(this.tfLogin, new VerticalLayout(this.cbAtivo, this.cbExterno, this.cbAdministrador)) ));	
		}else{
			this.tfNome.setEnabled(false);
		}
		this.adicionarCampo(Arrays.asList( new HorizontalLayout(this.tfNome, this.tfEmail) ));
